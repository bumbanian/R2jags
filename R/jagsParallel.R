###Code adapted from R2jags, https://github.com/suyusung/R2jags
...
{
  #Updated to use doParallel
  registerDoParallel(n.cluster)
  if(is.null(inits)){
    res = foreach(i = 1:n.cluster) %dopar% {
      jags(data = data, parameters.to.save = parameters.to.save, 
  ...
  #    jagsfit <- R2jags::jags(data = eval(expression(data)), inits = jags.inits,
  ...
  result <- as.bugs.array2(result, model.file = model.file,
                           program = "jags", DIC = TRUE, n.iter = n.iter, 
                           n.burnin = n.burnin, n.thin = n.thin)
  out <- list(model = model, BUGSoutput = result, parameters.to.save = parameters.to.save,
              model.file = model.file, n.iter = n.iter, DIC = TRUE)
